...
def save_region_variable_contrib(
    contribution, region, path_to_hdf_file, path_to_doc_folder
):
    ...
    if region == "GB BZN" and ("gen_other" in contribution.columns):
        corrected_gen_other = contribution.gen_other.add(
            contribution.gen_biomass, fill_value=0
        )
    ...
    if not contribution.empty:
        ...
        for var in contribution.columns:
            ...
            contribution.loc[:, var].to_hdf(
                path_to_hdf_file, key=contrib_name, mode="a", complevel=9
            )

            # Extract infos, e.g. nan-ratio
            contrib_info.loc[contrib_name, "nan_ratio"] = (
                contribution.loc[:, var].isnull().sum() / contribution.shape[0]
            )
            contrib_info.loc[contrib_name, "variable"] = var
            contrib_info.loc[contrib_name, "region"] = region
            contrib_info.loc[contrib_name, "number_neg_vals"] = (
                contribution.loc[:, var] < 0
            ).sum()
            ...
            plt.savefig(
                path_to_doc_folder + contrib_name + ".png", dpi=200, bbox_inches="tight"
            )
            plt.close()
...
def extract_region_variable_contrib(
    entsoe_data_folder,
    file_type,
    region,
    variable_name,
    ...
    contribution = pd.DataFrame(
        columns=column_rename_dict.values(), index=time_index, data=np.nan
    )

    # Iterate over files containing different months and years
    for date_index in pd.date_range(start_time, end_time, freq="ME"):

        file = entsoe_data_folder + "{}_{:02d}_{}.csv".format(
            date_index.year, date_index.month, file_type
        )
        ...
        if region == "DE-LU BZN":
            data = data[
                (data.AreaName == "DE-AT-LU BZN") | (data.AreaName == "DE-LU BZN")
            ]
        else:
        ...
        if pivot_column_name:
            data = data.pivot(
                index="DateTime", columns=pivot_column_name, values=variable_name
            )
...
def calc_mean_bzn_load(entsoe_data_folder, region, start_time, end_time):
    ...
    for date_index in pd.date_range(start_time, end_time, freq="ME"):
        ...
        file = entsoe_data_folder + "{}_{:02d}_ActualTotalLoad_6.1.A.csv".format(
...
def aggregate_external_features(
    region_contrib_path,
    mean_bzn_load,
    contrib_info,
    output_data,
    ...
    omitted_contribs = pd.DataFrame(
        columns=["region", "variable", "mean", "ratio_var", "ratio_load"]
    )
    ...
    for contrib_name, contrib in contrib_info.sort_values("nan_ratio").iterrows():
        ...
        if final_nan_ratio < final_nan_ratio_limit:
            ...
            potential_nan_ratio = (
                potential_input_data.isnull().any(axis=1) | invalid_outputs
            ).sum() / length
        ...
        if potential_nan_ratio < final_nan_ratio_limit:
            ...
            print(
                "Added: ",
                contrib_name,
                " | final nu. of points: {}".format(
                    length - int(final_nan_ratio * length)
                ),
            )
        ...
        # If potential (new) nan-ratio is too high, omitting new contribution
            ...
            omitted_contribs.loc[contrib_name] = contrib.loc[
                ["region", "variable", "mean"]
            ]
            print(omitted_contribs.head())
            ...
            if contrib.variable in raw_input_data:
                ...
                omitted_contribs.loc[contrib_name, "ratio_var"] = (
                    contrib.loc["mean"] / raw_input_mean
                )
            omitted_contribs.loc[contrib_name, "ratio_load"] = (
                contrib.loc["mean"] / raw_input_data.load.mean()
            )

            print(
                "Omitted: ",
                contrib_name,
                " | potential final nu. of points: {}".format(
                    length - int(potential_nan_ratio * length)
                ),
            )

    # Apply weighted average to prices
    raw_input_data.loc[:, "prices_day_ahead"] = (
        raw_input_data.prices_day_ahead / total_mean_load
    )
