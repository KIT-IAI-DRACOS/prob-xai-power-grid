...
# old entsoe_data_folder = "../External_data/ENTSO-E/"
entsoe_data_folder = "../External_data/ENTSO-E-20-24/"

# Path for generated features and tragets
# old folder = "../data/{}/"
folder = "../data/2020-2024/{}/"
...
# (Excluded 'RO', 'BG' as these countries do not (always) have Euro-data within our time frame.
...
bzn_area_names = {
    ...
    "SE": {"BZN": ["SE1 BZN", "SE2 BZN", "SE3 BZN", "SE4 BZN"]},
    "GB": {"BZN": ["GB BZN"]},
    "CH": {"BZN": ["CH BZN"]},
}
    ...
    for file_type, file_type_props in file_props.items():
        ...
        for region_type, region_list in file_type_props["regions"][area].items():
            ...
            for region in region_list:
                ...
                reg_var_contribution = extract_region_variable_contrib(
                ...
                # replaced since append is deprecated
                contrib_info = pd.concat([contrib_info, region_contrib_info])
...
for area in areas:
    ...
    mean_bzn_load = mean_bzn_load.squeeze("columns")
...
for area in areas:
    ...
    try:
        ...
        plt.legend(bbox_to_anchor=(0.8, 0.5, 0.5, 0.5))
        plt.savefig(
            doc_folder.format(area) + "omitted_contributions_relative_to_mean_load.svg",
            bbox_inches="tight",
        )
