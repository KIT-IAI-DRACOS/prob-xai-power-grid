...
for area in areas:
    ...
    for target in targets:
        print(f"-------- {target} --------")
        data_folder = f"../data/2020-2024/{area}/version_{data_version}/{scaler}/"
        fit_folder = f"../results/model_fit/{area}/version_{results_version}_tabnet_proba_final/{scaler}/target_{target}/"

        scaled_str = "_scaled"
        res_folder = f"../results/model_fit/{area}/version_{results_version}_tabnet_proba_final/{scaler}/target_{target}/explanations_partition/"
        ...
        mean_explainer = shap.PartitionExplainer(model.predict_mean, masker)
        variance_explainer = shap.PartitionExplainer(model.predict_std, masker)
