...
for area in areas:
    ...
    input_forecast = raw_input_data.loc[
        :, raw_input_data.columns.intersection(forecast_cols)
    ]
    input_actual = raw_input_data.loc[
        :, raw_input_data.columns.intersection(actual_cols)
    ]
    ...
    async_gen_cols = []
    for col in ["gen_solar", "gen_wind_off", "gen_wind_on"]:
        if col in input_actual.columns:
            async_gen_cols.append(col)
    input_actual["synchronous_gen"] = input_actual.total_gen - input_actual.loc[
        :, async_gen_cols
    ].sum(axis=1)
    ...
    input_forecast["total_gen_ramp_day_ahead"] = (
    ...
    if "wind_off_day_ahead" in input_forecast.columns:
        input_forecast["wind_off_ramp_day_ahead"] = (
